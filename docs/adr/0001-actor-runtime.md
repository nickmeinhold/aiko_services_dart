...
| presence | MQTT **last will** `(absent)`, **NOT retained** (`process.py:169` passes `retain_lwt=False`; `mqtt.py:66-74` position 5), published on `{namespace}/{host}/{pid}/0/state` when a process dies ungracefully. Only peers subscribed *at the moment of death* see it — a late joiner learns nothing, and asks the registrar instead. Contrast the **registrar's** will, which IS retained: `(primary absent)` on `{ns}/service/registrar` (`registrar.py:189-190` passes `True`), because primacy must be discoverable by a late joiner. A clean `disconnect()` does *not* fire either — though only when the client actually gets its DISCONNECT packet out |
