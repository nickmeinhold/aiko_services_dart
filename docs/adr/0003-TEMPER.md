...
**Overall verdict: DISSOLVE.** Two of three seated families voted DISSOLVE, which is decisive.
The candidate is invalidated. This is an honest negative result, not a failure to paper over —
and the strike produced the replacement, not merely a rejection.

**Struck:** `dt-1788521696`. Families seated: Maxwell (Claude), Kelvin (Gemini 2.5 Pro),
Carnot (GPT). Tesla (Grok) did not produce a verdict — **dark seat, coverage gap, not a SOUND
vote**. Wu (Kimi K3) disabled.
...
| Tesla (Grok) | — | dark seat; produced no verdict |
...
- **`ReplyError` belongs to the owning interface's spec, not to the framework.** — Carnot
  (uniquely; Maxwell had listed this under "what holds"). §2: a failed request produces an error
...
- **Correction 3 is itself an overclaim.** — Carnot (uniquely). The ADR "corrected" reply-N-times
...
All three families arrived at the same shape from different biases:
...
It does **not** delete the reply-topic blast-radius, and it does not answer whether a finite
multi-message reply is pattern 2 or pattern 3.
