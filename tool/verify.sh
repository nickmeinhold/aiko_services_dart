...
step "format"
# spike/ AND example/. analyze already walks the spikes (the 27-import surprise
# on a fresh clone) and their suites run, so leaving format out kept them
# half-in — the option PR #6 already called clearly wrong, and unformatted files
# there are evidence the gate never covered them rather than clutter to tidy
# past. `example/` joins for the same reason: it holds a real program (the bus
# observer), not scratch.
FORMAT_DIRS="lib/ test/ tool/ benchmark/ example/ spike/"
